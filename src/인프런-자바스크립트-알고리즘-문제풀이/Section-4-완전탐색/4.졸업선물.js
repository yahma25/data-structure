...
*/
export function solution_졸업선물(m, products) {
  let answer = 0;
  const SALE_DIVISION = 2;

  // 낮은 것부터 증가시키기 위해 가격 기준 오름차순 정렬
  products.sort((a, b) => a[0] + a[1] - (b[0] + b[1]));

  for (let i = 0; i < products.length; i++) {
    const product = products[i];
    let price = product[0] / SALE_DIVISION + product[1];
...
/*
예산 - 합산 결과가 가장 낮은 것 찾기 = 가장 예산에 가깝게 최대 효율
첫 번째부터 마지막 상품까지 1개씩 할인해보면서 합산하기
*/
