...
*/
export function solution_멘토링_my_thinking(test) {
  let answer = 0;
  let testCnt = test.length;
  let peopleCnt = test[0].length;
  let rankCnt = peopleCnt;
    ...
    for (let j = 1; j <= peopleCnt; j++) {
      ...
      for (let k = 0; k < testCnt; k++) {
        let mentorRank = 0,
          menteeRank = 0;
        // 등수 (1 ~ 4)
        for (let s = 0; s < rankCnt; s++) {
          // k번째 시험의 s등
          if (test[k][s] === i) mentorRank = s;
          if (test[k][s] === j) menteeRank = s;
        }

        // 멘토는 멘티보다 등수가 높아야 함 (숫자가 낮음 = 등수 높음)
