...
41
*/
export function solution_홀수(arr) {
  let answer = [];
  let sum = 0;
  let min = 100;
...
export function solution_홀수_built_in_function(arr) {
  let answer = [];

  const odds = arr.filter((num) => num % 2 === 1);
  answer.push(odds.reduce((total, num) => (total += num), 0));
  answer.push(Math.min(...odds));
  return answer;
}
